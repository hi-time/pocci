...
config ディレクトリの作成
-------------------------
サービス開始する前に、config ディレクトリに設定ファイルを作成する必要があります。

`bin/create-config` コマンドを実行することで、
config ディレクトリに以下のような設定ファイルが作成されます。
...
config ディレクトリは、次の3種類の方法のいずれかで作成できます。

1.  デフォルトの構成で作成する
2.  サービス構成タイプを指定して作成する
3.  サービス構成をカスタマイズして作成する
...
指定されたタイプの構成で設定ファイルを作成することができます。
